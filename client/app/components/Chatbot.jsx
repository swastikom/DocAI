...
const Chatbot = (props) => {
  ...
  const handleMessageSend = async () => {
    ...
    try {
      ...
      const response = await axios.post(`https://docai-zk2t.onrender.com/answer/${documentId}/`, {
        question: inputText
      });
  ...
  return (
    <div className="w-[70%] mx-auto p-4 flex flex-col h-screen pt-[6em]">
      {showWelcome && (
        <Welcome/>
      )}
      {!showWelcome && (
        <>
          <div ref={chatWindowRef} className="flex-grow chat-window bg-white rounded p-4 no-scrollbar overflow-y-auto">
            {messages.map((msg, index) => (
              <div
                key={index}
                className={`message p-2 rounded ${msg.fromUser ? 'bg-none py-4' : 'bg-none py-4'}`}
              >
            ...
            {error && <div className="px-5 py-2 my-2 text-white bg-red-500 rounded-lg w-fit">{error}</div>}
          </div>
          <div className='pb-7 sticky bottom-0 bg-white'>
            <div className="flex mt-2 shadow-md rounded-md">
              ...
              />
              <button
                className="bg-[#f6f7f9] border-[1px] border-l-0 text-slate-400 font-bold py-4 pl-1 pr-5 rounded-r-lg"
