...
def answer_question_gemini(text, question):
    text_splitter = CharacterTextSplitter(
        separator="\n",  # Split the text at newline characters
    ...
    texts = text_splitter.split_text(text)  # Split the raw text into chunks
    embeddings = GoogleGenerativeAIEmbeddings(model="models/embedding-001")
    document_search = FAISS.from_texts(texts, embeddings)
    docs = document_search.similarity_search(question)
    prompt = "Your name is DocAI. If and only if I say thank you to you only then just bid me welcome, otherwise can you answer question based on the provided pdf text. My question is: "+question + "also I got the following text from the document for you help. Following text: "+str(docs[0])
...
# def answer_question_gemini(text, question):
#   prompt_template = """Answer the question using the provided Text. If the answer is
